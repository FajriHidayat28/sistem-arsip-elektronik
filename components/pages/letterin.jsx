...
export default function LetterOut() {
  ...
  const handleDelete = async (fileid, file) => {
    await axios.delete("/api/files/delete/by-fileid?fileid=" + fileid + "&file=" + file).then((res) => {
      toastAlert("success", "Berhasil hapus!", "Penghapusan Surat berhasil!", 3000);
        ...
        <DataLists
          ...
          columns={[
            ...
            { field: "filesizeFormatted", header: "Ukuran Berkas" },
            {
              field: "filetype",
              header: "Tipe",
              body: (rowData) => <>{rowData.filetype === "LETTERIN" ? <i className="fa-regular fa-circle-down text-green-500"></i> : ""}</>,
            },
            {
              field: "actions",
              header: "Aksi",
