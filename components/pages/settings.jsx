...
export default function Settings() {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    if (name === "") {
      toastAlert("warning", "Penggantian Nama Gagal!", "Form tidak boleh kosong!", 3000);
    } else {
      await axios
  ...
  const handleUpdatePassword = async (e) => {
    ...
    if (password === "" || confirmPassword === "") {
      toastAlert("warning", "Kata Sandi Gagal!", "Form tidak boleh kosong!", 3000);
    } else if (password !== confirmPassword) {
      toastAlert("warning", "Kata Sandi Gagal!", "Konfirmasi kata sandi harus sama dengan kata sandi!", 3000);
    } else {
      await axios
        .put("/api/users/put/by-userid", {
    ...
    <>
      <div className="card flex flex-col gap-3">
        ...
        <form onSubmit={handleUpdateProfile} className="flex flex-col gap-3">
          ...
          <button type="submit" className="bg-blue-500 p-2 text-white ms-auto rounded-lg">
            Simpan
          </button>
        </form>
      </div>
        ...
        <form onSubmit={handleUpdatePassword} className="flex flex-col gap-3">
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex flex-col gap-2 basis-1/2">
              ...
              <input type={showPassword ? "text" : "password"} placeholder="Kata Sandi Baru" value={password} onChange={(e) => setPassword(e.target.value)} />
            </div>
            <div className="flex flex-col gap-2 basis-1/2">
              <label className="text-sm">Konfirmasi Kata Sandi</label>
              <input type={showPassword ? "text" : "password"} placeholder="Konfirmasi Kata Sandi Baru" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
            </div>
          </div>
          <div className="flex gap-3">
            <input type="checkbox" id="show-password" checked={showPassword} onChange={(e) => setShowPassword(e.target.checked)} />
      ...
      <div className="card mt-3 flex flex-col gap-3">
        ...
        <hr />
        <button type="button" onClick={() => signOut()} className="bg-red-500 p-2 text-white ms-auto rounded-lg">
          Keluar
        </button>
      </div>
    </>
